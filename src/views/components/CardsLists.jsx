      ...
      <Container expand='md' style={{ display: 'flex', alignItems: 'end' }}>
        ...
        <Dropdown style={{ height: '2.5rem' }} >
          ...
          <Dropdown.Menu>
            ...
            {
              reputations.map((rep) => {
                return (<Dropdown.Item key={`reputation_${rep.id}`} onClick={e => reputationFilterHandleOnChange(rep.id)}>{rep.name}</Dropdown.Item>)
              })
            }
          </Dropdown.Menu>
        </Dropdown>
          ...
          <Dropdown.Menu>
            <Dropdown.Item onClick={e => professionFilterHandleOnChange(e)}>All</Dropdown.Item>
            {
              professions.map((prof) => {
                return (<Dropdown.Item key={`prof_drop_down${prof.name}`} onClick={e => professionFilterHandleOnChange(e)}>{prof.name}</Dropdown.Item>)
              })
            }
          </Dropdown.Menu>
        </Dropdown>
