...
export default function CartOffItem({
  professional,
  totalDays,
  days,
  exitCanvas,
  ...
  const daysHandler = (e) => {
    if (days.includes(e.target.innerText))
      return setAlert({ day: `${e.target.innerText}`, show: true });
  ...
  return (
    <Card bg="info" style={{ width: "18rem" }}>
      ...
      >
        ...
        <Button variant="info" onClick={() => dispatch(deleteItemCart({professional, idDb}))}>
          Eliminar
        </Button>
      </Card.Header>
      <ListGroup variant="flush">
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              {availableDays.map((day) => {
                return (
                  <Dropdown.Item onClick={daysHandler}>{day}</Dropdown.Item>
                );
              })}
            </Dropdown.Menu>
          </Dropdown>
        ...
        <ListGroup.Item style={{ cursor: "pointer" }}>
          {days.map((day) => (
            <Badge onClick={() => dispatch(removeDayFromProf(id, day))}>
              {day}
            </Badge>
          ))}
        </ListGroup.Item>
      ...
      <ToastContainer>
        <Toast
          ...
          show={alert.show}
          onClose={() => setAlert({ day: "", show: false })}
        >
          <Toast.Header>
            <strong className="me-auto">No puedes realizar esta accion</strong>
          </Toast.Header>
          <Toast.Body>
            El dia {alert.day} para el profesional {firstName} {lastName} ya ha
            sido reservado
          </Toast.Body>
        </Toast>
      </ToastContainer>
      <ToastContainer>
        <Toast
          ...
          position="bottom-start"
          show={noDaysAlert}
          onClose={() => setNoDaysAlert(false)}
        >
          <Toast.Header>
            <strong className="me-auto">No puedes realizar esta accion</strong>
          </Toast.Header>
          <Toast.Body>
            Debes seleccionar los dias de tu profesional para seguir navegando
          </Toast.Body>
        </Toast>
      </ToastContainer>
    </Card>
